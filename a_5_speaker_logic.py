...
def _is_empty_or_punctuation_only(text: str) -> bool:
    ...
    Returns:
        True якщо текст порожній або містить лише розділові знаки
    """
    if not text or not text.strip():
        return True
    
    # Прибрати всі пробільні символи
    cleaned = text.strip()
    
    # Список всіх можливих розділових знаків
    punctuation_chars = {
        '"', "'", '"', '"', ''', ''', '«', '»', '„', '"',  # лапки
        '.', ',', ';', ':', '!', '?', '…', '—', '–', '-',  # розділові
        '(', ')', '[', ']', '{', '}', '<', '>',            # дужки
    ...
    # Перевірити чи всі символи є розділовими знаками або пробілами
    for char in cleaned:
...
def parse_script_events(text: str, voices_flat: List[str], max_speakers: int = 30) -> List[dict]:
    ...
    for line_no, raw_ln in enumerate(lines, start=1):
        ...
        if m_voice:
            ...
            # КРИТИЧНО: Перевірка на порожній текст або лише розділові знаки
        ...
        # Звичайний текст для g1 - також перевіряємо на порожність
