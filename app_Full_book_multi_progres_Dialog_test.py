...
def _load_and_process_sfx(sfx_id: str, target_sr: int) -> Tuple[int, np.ndarray]:
    ...
    if sr != target_sr:
        ...
        sr = target_sr
    # Нормалізація гучності до normalize_dbfs (можна вимкнути для конкретного SFX)
    normalize_dbfs = cfg_all.get('normalize_dbfs')
    # Якщо у конкретного ефекту явно вказано normalize: false — відключаємо нормалізацію
